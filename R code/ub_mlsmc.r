...
Options_unbiased = list(
  ...
  unbia_smc_Pmax = 0 ,  # Pmax when do unbiased algorithm with infinity Lmax, set Pmax as fixed
...
Options_SGD = list(
  ...
  SGD_length = 80000,  # total steps K in SGD
  run_SGD_MLSMC = FALSE , # (bool) run SGD
  epsilon_mlsmc_sgd = sqrt(2^(-c(12,13,14,15,16,17,18,19,20,22))),  # epsilon when do MLSMC
  mlsmc_l_upper_sgd = 3  # l_upper when calculate mlsmc
)

Param = list(
  cores = 8 ,  # parall cores
