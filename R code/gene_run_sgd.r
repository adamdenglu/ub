...
unbiased_est_withtheta <- function(param, mod_param, option, option_unbiased, option_sgd, mytheta, yt, Lmax, N_epsilon){
  ...
  if(option_sgd$unbiased_index == 1 | option_sgd$unbiased_index == 2){  # run unbiased estimator or with infinity Lmax
    unbiase_record <- foreach(x=1:N_epsilon, .packages=c("smccbase15",'lubridate')) %dopar% {source("smc_body.r");source("smc_base.r");unbiase_generate_withtheta(param, mod_param, option, option_unbiased, yt, Lmax, L_prob, mytheta)} 
  }else if(option_sgd$unbiased_index == 3 | option_sgd$unbiased_index == 4){  # run unbiased coupled sum estimator or with infinity Lmax
    unbiase_record <- foreach(x=1:N_epsilon, .packages=c("smccbase15",'lubridate')) %dopar% {source("smc_body.r");source("smc_base.r");unbiase_generate_coupled_withtheta(param, mod_param, option, option_unbiased, yt, Lmax, L_prob, mytheta)}
  }
...
generate_and_run_SGD_unbiased <- function(param, mod_param, option, option_sgd, option_unbiased){
  ...
  if(option_sgd$unbiased_index == 1 | option_sgd$unbiased_index == 3){
    ...
    Lmax = 10  # upper bound of l when calculate unbiased estimator for infinity Lmax
  }
...
# }
# 
# SGD_r_theta_log <- function(mytheta, y){
...
#   log_r = 0.5 * (big_M-1) * log(mytheta) - 0.5*mytheta*(sum(y^2)-sum(G_u*y)^2/sum(G_u^2))
#   
...
# }
# 
# SGD_example_reference1 <- function(option, option_sgd){
...
#   sgd_mle = optim(theta_ini, SGD_r_theta_log, y=Yt, method='BFGS')
#   write.table(sgd_mle$par, paste0(workdir2, '/SGD_example_truevalue.txt'))
#   return (sgd_mle)
# }
# 
# 
# SGD_r_theta_log_derivative <- function(mytheta, y){
#   # derivative of log likelihood
#   big_M = length(y)  # dimension of data
#   gp_x = seq(1/(big_M+1), 1-1/(big_M+1), by=1/(big_M+1))  # G(u)=[ p(x1;u),p(x2;u),\dots,p(x_M;u)]
#   G_u = sapply(gp_x, function(z) {0.5*(z^2-z)})
#   log_r = 0.5 * (big_M-1) / mytheta - 0.5 * (sum(y^2)-sum(G_u*y)^2/sum(G_u^2))
#   
#   s1 = sqrt(0.5*mytheta) * sqrt(sum(G_u^2)) * (1-sum(G_u*y)/sum(G_u^2)) 
#   s2 = sqrt(0.5*mytheta) * sqrt(sum(G_u^2)) * (-1-sum(G_u*y)/sum(G_u^2))
#   
...
#   
#   
#   sgd_mle = uniroot(SGD_r_theta_log_derivative, interval=c(0,100), y=Yt)
